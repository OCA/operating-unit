...
class TestOperatingUnitWindowActionIsolation(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.partner_a = cls.env["res.partner"].create({"name": "OU A Partner"})

        cls.partner_b = cls.env["res.partner"].create({"name": "OU B Partner"})
        ...
        settings_group = cls.env.ref("base.group_system")
        ...
        cls.user_without_default_ou = cls.env["res.users"].create(
            {
                ...
                "operating_unit_ids": [(6, 0, [cls.ou_a.id, cls.ou_b.id])],
            }
        )

        # sale.order contains operating_unit_id
        cls.action_without_domain = cls.env["ir.actions.act_window"].create(
            {
                "name": "Sale Orders",
        ...
        cls.action_with_domain = cls.env["ir.actions.act_window"].create(
            {
        ...
        cls.action_without_ou_field = cls.env["ir.actions.act_window"].create(
            {
                "name": "Countries",
    ...
    def test_user_without_default_ou(self):
        ...
        values = self.action_without_domain.with_user(
            self.user_without_default_ou
        ).read()[0]
    ...
    def test_model_without_operating_unit_field(self):
        ...
        values = self.action_without_ou_field.with_user(self.user_a).read()[0]

        self.assertFalse(values.get("domain"))

    def test_action_without_domain(self):
        """OU domain must be created."""
        values = self.action_without_domain.with_user(self.user_a).read()[0]

        domain = self._normalize_domain(values.get("domain"))
    ...
    def test_ou_b_domain_added(self):
        ...
        values = self.action_without_domain.with_user(self.user_b).read()[0]
    ...
    def test_existing_domain_preserved(self):
        ...
        values = self.action_with_domain.with_user(self.user_a).read()[0]
