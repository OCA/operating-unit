...
DOMAIN_TEMPLATE = (
    "['|',"
    "('{field}','=',False),"
    "('{field}','in', user.allowed_operating_unit_ids.ids)]"
)
RULE_NAME_TEMPLATE = (
...
RULE_XMLID_TEMPLATE = (
    "operating_unit.allowed_operating_unit_for_{model._table}_rule"
)
...
class OperatingUnitIrModel(models.Model):
    ...
    def _toggle_operating_unit_ir_rule(self, active=True):
        ...
        existing = self.env.ref(new_rule.xmlid, raise_if_not_found=False)
        if existing:
        ...
        new_rule.model_id = self.env['ir.model'].search([('model', '=', model._name)])
    ...
    def _toggle_operating_unit_ir_ui_view(self, active=True):
        ...
        for view in model_id.view_ids.filtered(
            ...
            new_view.res_id = self.env['ir.ui.view'].create({
                ...
                'mode'       : 'extension',
                'active'     : active,
                'groups_id'  : [(6, 0, view.groups_id.ids)],
                # fmt: on
            }).ensure_one()
            self.env['ir.model.data']._update_xmlids([{
