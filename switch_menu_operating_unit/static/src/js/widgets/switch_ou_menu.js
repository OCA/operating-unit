...
var SwitchOuMenu = Widget.extend({
    ...
    willStart: function () {
        var self = this;
        this.allowed_ou_ids = String(session.user_context.allowed_operating_unit_ids)
                                    .split(',')
        ...
        this.current_ou_name = _.find(session.user_operating_units.allowed_operating_units, function (ou) {
    ...
    _onSwitchOuClick: function (ev) {
        if (ev.type === 'key_down' && ev.which != $.ui.keyCode.ENTER && ev.which != $.ui.keyCode.SPACE) {
            return;
        }
        ev.preventDefault();
        ev.stopPropagation();
    ...
    _onToggleOuClick: function (ev) {
        ...
        if (dropdownItem.find('.fa-square-o').length) {
            allowed_ou_ids.push(companyID);
            dropdownItem.find('.fa-square-o').removeClass('fa-square-o').addClass('fa-check-square');
            $(ev.currentTarget).attr('aria-checked', 'true');
        } else {
            allowed_ou_ids.splice(allowed_ou_ids.indexOf(companyID), 1);
